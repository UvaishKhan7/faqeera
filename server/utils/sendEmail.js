...
const sendOrderConfirmationEmail = async (customerEmail, orderDetails) => {
  try {
    const { data, error } = await resend.emails.send({
      from: 'Faqeera Store <onboarding@resend.dev>', // Do not change this for testing
      to: [customerEmail],
...
const sendShippingConfirmationEmail = async (customerEmail, orderDetails) => {
  const trackingNumber = orderDetails.shippingInfo?.trackingNumber; // Safely access the tracking number
...
module.exports = { sendOrderConfirmationEmail, sendShippingConfirmationEmail };